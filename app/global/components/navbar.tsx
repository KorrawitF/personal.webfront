    ...
    return (
        ...
        <nav className="fixed w-full z-20 top-0 start-0 border-default">
          <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 2xl:w-1/2">
            ...
            <div className="hidden w-full font-bold md:block md:w-auto bg-background border border-2 rounded-full" id="navbar-default">
              <ul className="font-medium flex flex-row flex-nowrap gap-1 justify-center md:p-0 mt-4 border border-default rounded-base bg-neutral-secondary-soft md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-neutral-primary">
                {menus.map((menu) => {
                    ...
                    return (
                        <li key={menu.name} className={`text-center py-1 px-4 m-0 rounded-full ${isActive ? 'bg-primary border-special border-2' : 'border border-transparent'}`}>
                            <Link 
                            key={menu.name}
                            href={menu.href}
                            className={`text-center block bg-brand rounded md:bg-transparent md:text-fg-brand md:p-0 ${isActive ? 'text-background font-bold' : 'text-white hover:text-secondary'}`}
                            >
                                {menu.name}
                            </Link>
                        </li>
